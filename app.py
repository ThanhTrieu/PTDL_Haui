...
def read_data():
    ...
    df['engineSize'] = pd.cut(df['engineSize'],
                              bins=[-1, df['engineSize'].quantile(0.25) - 0.01, df['engineSize'].quantile(0.5) - 0.01,
                                    df['engineSize'].quantile(0.75) - 0.01, df['engineSize'].max() + 0.01],
...
def run_app(intercept, coef, preprocessor, ols_scaler, results, df):
    ...
    def predict():
        ...
        return render_template('index.html', prediction=prediction, image_source=image_source)
